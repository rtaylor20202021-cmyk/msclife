...
// Practice
function practice() {
  if (money < 20) { print("Not enough money ($20 needed)"); return; }
  money -= 20;
  skill += 5;
...
// Make Album with selection
function makeAlbum() {
  let unreleased = songs.filter(s => !s.released);
  if (unreleased.length < 3) {
    print("Need at least 3 unreleased songs.");
    return;
  }
  ...
  for (let i = 0; i < max; i++) {
    let choice = parseInt(prompt(`Select song ${i+1}/${max} (enter number from your songs list):`)) - 1;
    if (choice >= 0 && choice < songs.length && !songs[choice].released) {
      selected.push(songs[choice]);
    }
  }
  if (selected.length < 3) {
    print("Not enough songs selected.");
    return;
  }
...
function startTour() {
  ...
  let type = prompt("Tour type: 1=Regional 2=National 3=Continental 4=Global");
...
print("Musician Manager - Button Version<br>Use the buttons below.");
updateStatus();
